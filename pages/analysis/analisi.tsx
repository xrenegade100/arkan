...
const analisi: NextPage = () => {
    ...
    (async () => {
      if (pojo) {
        if (pojo !== '{}') {
          ...
          setAnalysisExist(validation.VALID);
        } else {
          setAnalysisExist(validation.EMPTY);
        }
      } else {
        setAnalysisExist(validation.INVALID);
      }
    })();
  }, []);
  ...
  if (
    analysisExist === validation.VALID ||
    analysisExist === validation.EMPTY
  ) {
    if (analysis || analysisExist === validation.EMPTY) {
      return (
        <div
          className={clsx('w-full flex flex-col justify-center items-center', {
            'h-screen': analysisExist === validation.EMPTY,
          })}
        >
          <section className="mx-4 pt-24 pb-6 lg:w-4/5 lg:py-24 flex flex-col justify-start items-start">
            <span className="text-4xl md:text-5xl lg:text-6xl 2xl:text-8xl font-body font-bold">
              {analysisExist === validation.VALID
                ...
                : 'Nessun Dark Pattern Rilevato!'}
            </span>
            <span className="py-4 text-lg font-body text-justify lg:text-start">
              {analysisExist === validation.VALID
                ? "Dall'analisi effettuata sul sito risultano i seguenti darkpattern:"
                : "All'interno del sito non è stato rilevato alcun tipo di dark pattern tra quelli che sono analizzati tra i nostri algoritmi o rilevati automaticamente"}
            </span>
          </section>
          <section className="w-full lg:w-4/5 flex flex-col-reverse md:flex-row justify-between items-start">
            {analysisExist === validation.VALID && analysis && (
