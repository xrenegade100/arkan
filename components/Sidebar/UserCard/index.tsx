...
const UserCard: React.FC<Props> = ({ username, email, className }: Props) => {
    return (
        <div className={`${className} w-fit md:w-10/12 lg:w-11/12 bg-gradient-to-br from-secondary-main to-secondary-accent flex justify-center items-center py-4 px-2 shadow-md rounded-md`}>
            ...
            <div className="ml-2 flex flex-col justify-center items-start overflow-hidden">
                ...
                <span className="w-full truncate text-xs text-gray-700 font-body" title={email}>{email}</span>
            </div>
        </div>
    )
}
